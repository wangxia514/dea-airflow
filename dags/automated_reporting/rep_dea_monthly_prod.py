...
dag = DAG(
    ...
    description="DAG for dea processing - ungrouped user stats",
    tags=["reporting"],
...
with dag:
    ...
    fk4_ungrouped_user_stats_ingestion = KubernetesPodOperator(
        ...
        in_cluster=True,
        task_id="fk4_ungrouped_user_stats_ingestion",
        get_logs=True,
        env_vars={
            "EXECUTION_DATE": "{{ ds }}",
            "FILE_TO_PROCESS": "fk4",
    ...
    fk4_ungrouped_user_stats_processing = KubernetesPodOperator(
        namespace="processing",
        image="python:3.8-slim-buster",
        arguments=["bash", "-c", " &&\n".join(JOBS2)],
        name="fk4_ungrouped_user_stats_processing",
        do_xcom_push=False,
        is_delete_operator_pod=True,
        in_cluster=True,
        task_id="fk4_ungrouped_user_stats_processing",
        get_logs=True,
        env_vars={
            "AGGREGATION_MONTHS" : "{{ task_instance.xcom_pull(task_ids='fk4_ungrouped_user_stats_ingestion') }}",
            "EXECUTION_DATE": "{{ ds }}",
        },
    )
    rs0_ungrouped_user_stats_ingestion = KubernetesPodOperator(
        namespace="processing",
        image="python:3.8-slim-buster",
        arguments=["bash", "-c", " &&\n".join(JOBS3)],
        name="write-xcom",
        ...
        is_delete_operator_pod=True,
        in_cluster=True,
        task_id="rs0_ungrouped_user_stats_ingestion",
        get_logs=True,
        env_vars={
            "EXECUTION_DATE": "{{ ds }}",
            "FILE_TO_PROCESS": "rs0",
    ...
    rs0_ungrouped_user_stats_processing = KubernetesPodOperator(
        namespace="processing",
        image="python:3.8-slim-buster",
        arguments=["bash", "-c", " &&\n".join(JOBS4)],
        name="rs0_ungrouped_user_stats_processing",
        do_xcom_push=False,
        is_delete_operator_pod=True,
        in_cluster=True,
        task_id="rs0_ungrouped_user_stats_processing",
        get_logs=True,
        env_vars={
            "AGGREGATION_MONTHS" : "{{ task_instance.xcom_pull(task_ids='rs0_ungrouped_user_stats_ingestion') }}",
            "EXECUTION_DATE": "{{ ds }}",
        },
    )
