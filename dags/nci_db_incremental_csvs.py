...
with DAG(
    ...
    run_changes_csv_dump = SSHOperator(
        ...
        + dedent(
            ...
            table=agdc.dataset
            echo Dumping changes from $table
            psql --quiet -c "\\copy (select * from $table where updated <@ tstzrange('{{ prev_ds }}', '{{ ds }}') or archived <@ tstzrange('{{ prev_ds }}', '{{ ds }}') or added <@ tstzrange('{{ prev_ds }}', '{{ ds }}')) to program" -h ${host} -d datacube | gzip -c - > ${table}_changes.csv.gz
