...
class PBSJobOperator(SSHRunMixin, BaseSensorOperator):
    ...
    def execute(self, context):
        # Because this Operator is a 'reschedule' type Airflow Sensor, the Python class will be
        # created fresh each time it is attempted. We need a persistent storage location, and Airflow XCom is a
        # good choice.
        # TODO: Put pbs_job_id into the context
        pbs_job_id = self.xcom_pull(context, task_ids=[self.task_id], key=XCOM_KEY)
        context["pbs_job_id"] = pbs_job_id
        if pbs_job_id:
            ...
            if ret_val == 0:
                pbs_job_id = output.strip()
                self.xcom_push(context, key=XCOM_KEY, value=pbs_job_id)
            else:
    ...
    def poke(self, context):
        ...
        pbs_job_id = context["pbs_job_id"]
        try:
            ret_val, output = self.run_ssh_command_and_return_output(
                f"qstat -fx -F json {pbs_job_id}"
            )
        except EOFError:
        ...
        job_state = result["Jobs"][pbs_job_id]["job_state"]
        if job_state == "F":
            pbs_result = result["Jobs"][pbs_job_id]
            ...
            if exit_status == 0:
                ...
                raise AirflowException("PBS Job Failed %s", pbs_job_id)
        else:
            return False
