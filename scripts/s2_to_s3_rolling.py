...
def find_granules(_num_days, _end_date, root_path=NCI_DIR):
    ...
    dates = [
        (_end_date - timedelta(days=x)).strftime("%Y-%m-%d")
        for x in range(_num_days + 1)
    ]
...
def sync_granule(granule, _s3_bucket):
    ...
    s3_path = "s3://{s3_bucket}/{s3_path}/{granule}".format(
        s3_bucket=_s3_bucket, s3_path=S3_PATH, granule=granule
    )
...
def sync_dates(_num_days, _end_date, _s3_bucket, _update="no"):
    ...
    datetime_end = (
        dt.today() if _end_date == "today" else dt.strptime(_end_date, "%Y-%m-%d")
    )
        ...
        for granule in list_of_granules:
            ...
            if Path(yaml_file).exists():
                ...
                if sync_action != "no":
                    ...
                    if sync_success and (
                        sync_action in ("metadata", "granule_metadata")
                    ):
                        # Replace the metadata with a deterministic ID
                        replace_metadata(yaml_file, _s3_bucket, s3_metadata_path)
                        LOG.info(
                            "Finished processing and/or uploaded metadata to %s",
                            s3_metadata_path,
                        )
                    else:
                        LOG.error("Failed to sync data... skipping")
                        error_list.append(
                            f"Failed to sync {granule} because of an error in the sync command"
                        )
                else:
                ...
                error_list.append(
                    f"Failed to sync {granule} because of missing metadata file"
                )
    else:
...
@click.option(
    ...
    "-u",
    type=click.Choice(
        ["granule_metadata", "granule", "metadata", "no"], case_sensitive=True
    ),
    default="no",
)
def main(numdays, enddate, s3bucket, doupdate):
    ...
    LOG.info(
        "Syncing %s days back from %s into the %s bucket and update is %s",
        numdays,
        enddate,
        s3bucket,
        doupdate,
